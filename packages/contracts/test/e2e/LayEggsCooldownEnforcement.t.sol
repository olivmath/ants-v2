// SPDX-License-Identifier: MIT
pragma solidity >=0.8.4 <0.9.0;

import {Test} from 'forge-std/Test.sol';
import {CryptoAnts, ICryptoAnts} from 'src/CryptoAnts.sol';
import {Egg, IEgg} from 'src/Egg.sol';
import {TestUtils} from 'test/TestUtils.sol';

contract LayEggsCooldownEnforcementTest is Test, TestUtils {
  uint256 internal constant FORK_BLOCK = 9_817_118;
  ICryptoAnts internal _cryptoAnts;
  address internal _owner = makeAddr('owner');
  IEgg internal _eggs;

  function setUp() public {
    // Deploy contracts locally (no fork needed)
    uint64 currentNonce = vm.getNonce(address(this));
    address futureAntsAddress = vm.computeCreateAddress(address(this), currentNonce + 1);
    _eggs = new Egg(futureAntsAddress);
    _cryptoAnts = new CryptoAnts(address(_eggs), 'ipfs://bafkreic7mcysfc7eqlri2pjkfupkri7xk47c25hbwp4sm34owe3hudkkzm/');
  }

  function testLayEggsCooldownEnforcement() public {
    // Given: a user creates an ant
    address user = makeAddr('user');
    vm.deal(user, 1 ether);

    vm.prank(user);
    _cryptoAnts.buyEggs{value: 0.01 ether}(1);

    vm.prank(user);
    _eggs.approve(address(_cryptoAnts), type(uint256).max);

    vm.prank(user);
    _cryptoAnts.createAnt();

    // When: user tries to call layEggs immediately (cooldown not met)
    // Then: should revert (ant created with lastEggLayTime = 0, so need to wait)
    // Note: First egg laying requires waiting from creation time
    vm.expectRevert(ICryptoAnts.CooldownNotMet.selector);
    vm.prank(user);
    _cryptoAnts.layEggs(1);

    // And: after warping time past cooldown, layEggs should work
    vm.warp(block.timestamp + 601);

    vm.prank(user);
    _cryptoAnts.layEggs(1);

    // Verify ant is still alive (if it survived) before testing cooldown again
    try _cryptoAnts.ownerOf(1) returns (address) {
      // Ant survived - test cooldown enforcement
      vm.expectRevert(ICryptoAnts.CooldownNotMet.selector);
      vm.prank(user);
      _cryptoAnts.layEggs(1);

      // After warping again, should work
      vm.warp(block.timestamp + 601);
      vm.prank(user);
      _cryptoAnts.layEggs(1);
    } catch {
      // Ant died - cooldown test still passed on first attempt
      // Test is valid
    }
  }
}
